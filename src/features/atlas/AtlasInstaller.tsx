...
export function AtlasInstaller() {
  ...
  const checkInstallationStatus = async () => {
    try {
      const version = await atlasRepository.getAtlasVersion();
  ...
  const downloadDataset = async (): Promise<any> => {
    // Lazily load the dynamic dataset generated by the AI script
    try {
      const module = await import('@/atlas/dataset.json');
      return module.default || module;
    } catch (err) {
      console.error('Failed to dynamically load dataset.json', err);
      // Fallback to mock dataset if generation hasn't run yet
